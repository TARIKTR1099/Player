...
const NavItem = ({ icon, label, active, onClick, sidebarMode }) => (
  <div onClick={onClick} className={`flex items-center ${sidebarMode === 'icons' ? 'justify-center' : 'justify-start space-x-3'} p-3 rounded-xl cursor-pointer transition-all text-sm font-bold tracking-tight`} style={active ? {backgroundColor:'var(--color-primary)', color:'white'} : {color:'var(--text-secondary)'}}>
  ...
  return (
    <div className={`flex flex-col border-r flex-shrink-0 relative transition-all duration-200 overflow-hidden ${sidebarMode === 'collapsed' ? 'w-0 border-r-0' : ''}`} style={sidebarMode === 'collapsed' ? {} : {width: sidebarMode === 'icons' ? 64 : sidebarWidth, backgroundColor:'var(--color-bg-secondary)', borderColor:'var(--border-color)'}}>
      {sidebarMode !== 'collapsed' && (<>
       <div className={`flex items-center whitespace-nowrap ${sidebarMode === 'icons' ? 'p-4 justify-center' : 'p-6 gap-3'}`} style={{color:'var(--color-primary)'}}>
         {sidebarMode === 'icons' ? (
      ...
      <nav className={`flex-1 ${sidebarMode === 'icons' ? 'px-2' : 'px-4'} space-y-1`}>
      ...
      <div className={`border-t whitespace-nowrap ${sidebarMode === 'icons' ? 'px-2' : 'p-4'}`} style={{borderColor:'var(--border-color)'}}>
